...
void SD_SPI_Init(void)
{
  //设置硬件上与SD卡相关联的控制引脚输出
	...
	RCC_APB2PeriphClockCmd(	SD_SPI_GPIO_CLK|SD_CS_CLK, ENABLE );
	...
	GPIO_SetBits(SD_SPI_PORT,SD_SPI_SCK_PIN|SD_SPI_MISO_PIN|SD_SPI_MOSI_PIN);
...
u8 SD_Initialize(void)
{
	...
	SD_DisSelect();//取消片选
	SD_SPI_SpeedHigh();//高速
	if(SD_Type)return MMC_SDCard_Ready;
	else if(r1)return r1; 	   
	return MMC_SDCard_OtherErrors;//其他错误
}
//读SD卡
//buf:数据缓存区
//sector:扇区
